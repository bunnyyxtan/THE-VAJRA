...
/**
 * /receipt/[requestId] — receipt verification (Ledger grammar; Seal grammar
 * only at verified completion).
 *
 * Two independent evidence channels, each with its own pending state:
 *   A) contract state (statusOf + settlementOf) — the settlement record.
 *   B) the PaymentFulfilled event log — located by binary-searching block
 *      timestamps for settlement.paidAt, then scanning forward in ≤100-block
 *      chunks (the public Monad RPC caps eth_getLogs at 100 blocks).
 * The channels are cross-checked; a disagreement is a verification conflict,
 * never a silently merged record. Nothing is trusted from the URL beyond the
 * request ID itself.
 */
...
export function ReceiptView({ requestId }: { requestId: string }) {
  ...
  const client = useMemo(() => getVajraPublicClient(config), [config]);
  ...
  return (
    <div className="vscreen">
      <div className="vscreen__page vscreen__page--narrow receipt-page">
        <p className="receipt-print-only">
          VAJRA payment receipt · VajraNativeV1 at{" "}
          <span className="sc-mono">{config.contractAddress}</span> on Monad Mainnet (chain
          143)
        </p>

        <div className="vscreen__head">
          <h1 className="vscreen__title">Payment receipt</h1>
          {verifiedAt > 0 && (
        ...
        {!valid ? (
          ...
          <section className="receipt-empty" aria-busy={refreshing}>
            ...
            <p className="sc-note">
              ...
              <span>
                ...
                and call <span className="sc-mono">statusOf</span> /{" "}
                <span className="sc-mono">settlementOf</span> with this request ID.
              </span>
            </p>
          ...
          <section className="receipt-empty">
            ...
            {revoked ? (
              ...
              <p className="receipt-empty__body">
                The canonical VajraNativeV1 contract on Monad Mainnet shows no settled
                payment for this request ID. It may never have been paid, the ID may be
                mistyped, or the link may come from a different deployment.
              </p>
            )}
            <LedgerBlock title="Request">
              <LedgerRow
            ...
            <ol className="receipt-empty__steps">
              ...
              <li>
                Call <span className="sc-mono">statusOf(requestId)</span> —{" "}
                <span className="sc-mono">Paid</span> means settled;{" "}
                <span className="sc-mono">Unused</span> means no payment exists.
              </li>
              <li>
                Call <span className="sc-mono">settlementOf(requestId)</span> for the payer,
          ...
          <>
            ...
            <div className="receipt-reveal receipt-reveal--1">
              ...
              >
                ...
                <LedgerRow
                  label="Amount"
                  value={
                    <span className="sc-tnum" style={{ fontWeight: 700 }}>
            ...
            {eventCheck.phase === "done" && eventLog === null && (
              <p className="sc-note sc-note--warning">
                ...
                <span>
                  ...
                  transaction hash and block number are pending. The settlement record
                  above comes from contract state and is authoritative.
                </span>
            ...
            <p className="sc-note receipt-no-print">
              <InfoIcon />
              <span>
                Anyone can verify this receipt independently: call{" "}
                <span className="sc-mono">statusOf</span> /{" "}
                <span className="sc-mono">settlementOf</span> with this request ID on the{" "}
                <a href={contractExplorerUrl} target="_blank" rel="noreferrer">
                  canonical contract
                </a>
                .
...
function InvalidRequestId({
  ...
  return (
    <section className="receipt-empty">
      ...
      <p className="receipt-empty__body">
        A Vajra request ID is a 32-byte hex digest (<span className="sc-mono">0x</span>{" "}
        followed by 64 hexadecimal characters). The value in this link does not match that
        format, so nothing was queried onchain.
      </p>
      {raw.length > 0 && raw.length <= 128 && (
        <LedgerBlock title="Received">
          <LedgerRow label="Value" value={<span className="sc-break">{raw}</span>} mono />
        </LedgerBlock>
      )}
      <h3 className="receipt-empty__subtitle">How to verify independently</h3>
      <ol className="receipt-empty__steps">
...
function ReceiptSkeleton() {
  return (
    <>
      <p className="vscreen__freshness" role="status">
