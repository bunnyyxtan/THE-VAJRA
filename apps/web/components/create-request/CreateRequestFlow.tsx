...
export function CreateRequestFlow() {
  ...
  const wrongChain =
    mounted && isConnected && walletChainId !== MONAD_MAINNET_CHAIN_ID;

  // --- Proof chain status (7-stage motif; only the first three belong here)
  ...
  function validate(): FieldErrors {
    ...
    if (memoUtf8Bytes(memo).length > MAX_MEMO_BYTES)
      next.memo = `Memo is over the ${MAX_MEMO_BYTES}-byte protocol limit. Shorten it by ${memoUtf8Bytes(memo).length - MAX_MEMO_BYTES} byte${memoUtf8Bytes(memo).length - MAX_MEMO_BYTES === 1 ? "" : "s"}.`;
  ...
  function onReviewTerms() {
    ...
    const request = buildPaymentRequest({
      ...
      payer: payerTrimmed === "" ? ANY_PAYER : getAddress(payerTrimmed),
      amount: units,
      issuedAt: nowSeconds,
  ...
  return (
    <main className="request-page">
      <header className="request-header">
        <div>
          <h1 className="request-header__title">Create request</h1>
          <p className="request-header__sub">
            Signed terms, exact amount, one payment.
          </p>
        </div>
        <ThemeToggle />
      </header>
      ...
      <div className="request-grid">
        {/* ------------------------------------------------ Instrument column */}
        <div className="request-instrument">
          {wrongChain && (
            <Banner
              tone="warning"
              title="Wrong network"
            ...
            >
              Your wallet is connected to a different network. Switch to Monad
              Mainnet (chain ID {MONAD_MAINNET_CHAIN_ID}) before signing.
            </Banner>
          )}
          ...
          {step === "form" && (
            <>
              ...
              <div className="request-actions">
                <div className="request-actions__row">
                  <Button
                    className="request-actions__primary"
          ...
          {step === "review" && draft && (
            <>
              ...
              <RequestStages reached={stageReached} waiting={stageWaiting} />
              ...
              <div className="request-actions">
                <div className="request-actions__row">
                  ...
                  <Button variant="ghost" disabled={signing} onClick={() => setStep("form")}>
                    Back to edit
                  </Button>
                </div>
              </div>
          ...
          {step === "success" && draft && link && (
            <>
              <div className="request-success__head">
                ...
                <div>
                  <h2 className="request-success__title">Request created</h2>
              ...
              <RequestStages reached={4} waiting={false} />
        ...
        {/* ------------------------------------------------ Context column */}
        <aside className="request-context" aria-label="Request progress and environment">
          <div className="request-context__block">
            <h2 className="request-context__title">Proof chain</h2>
